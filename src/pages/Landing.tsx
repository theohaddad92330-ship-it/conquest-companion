...
import {
  ...
  Users, Mail, Shield, FileText, Layers, Clock,
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative pt-32 pb-32 px-6 overflow-hidden min-h-[90vh] flex items-center">
        ...
        >
          {/* Integration badges */}
          <motion.div custom={0} variants={fadeUp} className="flex flex-wrap items-center justify-center gap-3 mb-8">
            {integrationBadges.map((b) => (
              <div key={b.label} className="glass-badge inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-medium text-muted-foreground">
                <span>{b.icon}</span>
          ...
          {/* Headline */}
          <motion.h1 custom={1} variants={fadeUp} className="font-display text-5xl md:text-7xl lg:text-[80px] font-bold leading-[0.95] tracking-tight mb-6">
            Où l'intelligence
            <br />
          ...
          <motion.div custom={3} variants={fadeUp} className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Button size="lg" className="text-base px-8 h-13 bg-foreground text-background hover:bg-foreground/90 border border-border font-semibold" asChild>
              <Link to="/signup">Commencer gratuitement</Link>
            </Button>
            <Button variant="outline" size="lg" className="text-base px-8 h-13 glass border-border/50 hover:bg-muted/20" asChild>
        ...
        {/* Logo marquee */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8, duration: 0.6 }} className="absolute bottom-8 left-0 right-0 z-10">
          <div className="flex items-center justify-center gap-12 opacity-30">
            {partnerLogos.map((name) => (
              <span key={name} className="text-sm font-medium text-muted-foreground tracking-wide uppercase select-none">{name}</span>
            ))}
          </div>
        </motion.div>
      </section>
      ...
      <section id="how-it-works" className="py-24 px-6 border-t border-border/50">
        <div className="mx-auto max-w-5xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center mb-16">
            ...
            <motion.p variants={fadeUpSimple} className="text-muted-foreground max-w-lg mx-auto">3 étapes. À la fin, vous savez qui contacter, quoi dire, et dans quel ordre.</motion.p>
          </motion.div>
      ...
      <section id="features" className="py-24 px-6 border-t border-border/50">
        <div className="mx-auto max-w-4xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center mb-16">
            ...
            <motion.p variants={fadeUpSimple} className="text-muted-foreground max-w-lg mx-auto">Du recul pour décider, et des livrables prêts à utiliser.</motion.p>
          </motion.div>
      ...
      <section className="py-24 px-6 border-t border-border/50">
        <div className="mx-auto max-w-4xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            ...
            <motion.p variants={fadeUpSimple} className="text-center text-muted-foreground mb-4 max-w-lg mx-auto">Vous n'avez pas besoin de "plus de données". Vous avez besoin de savoir quoi faire, sans y passer la journée.</motion.p>
            <motion.p variants={fadeUpSimple} className="text-center text-muted-foreground/60 text-sm mb-10 max-w-md mx-auto">Bellum utilise votre profil ESN (offres, secteurs, personas, zone) pour sortir des recommandations qui collent au terrain.</motion.p>
            <motion.div variants={stagger} className="grid md:grid-cols-2 gap-4 max-w-2xl mx-auto">
              {[
                ...
                <motion.div key={item.title} variants={fadeUpSimple} className="glass-card p-4 flex items-start gap-3 card-hover">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={plan.name} variants={fadeUpSimple}>
                <Card className={`h-full glass-card relative ${plan.popular ? "border-primary bellum-glow" : ""}`}>
                  ...
                  <CardContent className="p-6 text-center space-y-4">
                    ...
                    <div><span className="font-display text-3xl font-bold">{plan.price}</span><span className="text-sm text-muted-foreground">{plan.period}</span></div>
        ...
        <div className="mx-auto max-w-5xl">
          <div className="grid md:grid-cols-4 gap-8">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Transformez un nom de compte en plan de conquête.</p>
            </div>
            <div>
              <h4 className="text-sm font-semibold mb-3">Produit</h4>
              <div className="space-y-2">
                <Link to="/features" className="block text-sm text-muted-foreground hover:text-foreground transition-colors">Fonctionnalités</Link>
            ...
            <div>
              ...
              <div className="space-y-2">
                <Link to="/help" className="block text-sm text-muted-foreground hover:text-foreground transition-colors">Support</Link>
              </div>
            </div>
            <div>
              <h4 className="text-sm font-semibold mb-3">Légal</h4>
              <div className="space-y-2">
                <Link to="/terms" className="block text-sm text-muted-foreground hover:text-foreground transition-colors">CGV</Link>
                <Link to="/privacy" className="block text-sm text-muted-foreground hover:text-foreground transition-colors">Confidentialité</Link>
                <Link to="/legal" className="block text-sm text-muted-foreground hover:text-foreground transition-colors">Mentions légales</Link>
              </div>
            </div>
          </div>
          <div className="mt-10 pt-6 border-t border-border/50 text-center text-xs text-muted-foreground/40">
